...
| **Tier 3 — Financial, destructive, or irreversible** | Money moves, data is permanently destroyed, or external systems are changed | Stripe refunds, subscription cancellations, permanent file deletion, sending money, account/security settings changes | **Confirmation required + read-back of exact details + cannot be enabled for "auto-approve" ever, AND a two-factor confirmation (R-2a)** |
...
- **R-2a (Tier 3 two-factor confirmation, decided)**: in addition to R-2's voice/UI read-back and spoken "confirm," Tier 3 actions ALSO require a **TOTP code from an authenticator app (e.g., Google Authenticator)**, entered in the UI (never spoken — a spoken code could be captured by a recording). Setup is one-time: JARVIS generates a TOTP secret on first use, the user scans the resulting QR code / enters the setup key into their authenticator app, and the secret is stored in the credential vault (never in plaintext config). Both factors — the voice "confirm" AND a valid, unexpired 6-digit code — must be present before a Tier 3 action executes. This satisfies SEC-6 (no spoken secret) while keeping the "voice + something else" experience the user asked for.
...
- **SEC-6: Voice spoofing awareness.** Since anyone who can speak to your microphone can issue commands, Tier 3 actions require **both** a spoken/UI "confirm" (R-2) **and** a TOTP authenticator code entered in the UI (R-2a) — a recorded or overheard voice alone can never authorize money movement or permanent deletion.
...
5. **Confirmation method for Tier 3** — voice phrase, UI click only, or PIN? *(Decided: voice "confirm" AND a Google Authenticator (TOTP) code — see R-2a/SEC-6.)*
