...
Top-level keys: `month`, `currency`, `months_in_ledger`, `is_baseline_month`, `cash_flow {income, personal_spend, business_spend, net}`, `freed_for_business {current, previous, mom_delta, definition, history[]}`, `categories[] {category, type, total, previous, mom_delta, avg_3mo, vs_avg}`, `business {total, by_merchant}`, `recurring[] {merchant, category, monthly_cost, months_seen, previous_cost, price_creep}`, `recurring_total_monthly`, `top_merchants[]` (actual merchants only — e-transfer payments to people are excluded), `big_ticket[]`, `budget_vs_actual[] | null`, `budget_proposal | null`, `uncategorized_count`, `usd_transactions[] {date, description, amount, category, business}`.

Notes: `price_creep` is only set for fixed-price categories (Subscriptions, Utilities/Phone, Health, Housing, Debt Payments) — a grocery basket costing more is not a price increase. In the ingest summary, `needs_input` counts transactions while `unknown_merchants`/`unknown_recipients` list unique names, so the numbers differ when one merchant has several transactions.
